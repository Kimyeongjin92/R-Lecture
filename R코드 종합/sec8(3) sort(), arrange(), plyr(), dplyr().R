...
Fruits %>% filter(Expenses>90 | Sales>80)       # a | b : 둘다 뽑아라
...
Fruits %>% group_by(Fruit) %>% summarise_each(funs(Sales=sum(Sales),n()),Sales)
Fruits %>% select(Fruit:Sales,-Location) %>% group_by(Fruit) %>% summarise(n=n())
